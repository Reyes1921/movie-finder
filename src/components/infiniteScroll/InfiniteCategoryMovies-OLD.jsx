...
import {Genre, Loading, MovieGrid} from ".."
import {Layout} from "../../layout/Layout"
import axios from "axios"
const apiBearer = import.meta.env.VITE_API_BEARER
import {useTranslation} from "react-i18next"

export const InfiniteCategoryMovies = ({title, id}) => {
  ...
  const fetchMoreData = () => {
    if (index <= 300) {
      axios
        .get(
          `https://api.themoviedb.org/3/discover/movie?with_genres=${id}&page=${index}&${
            language === "en" ? "language=en-US" : "language=es-ES"
          }`,
          {
            headers: {
  ...
  return (
    <Layout>
      ...
      <Genre language={language} mediaType={"movie"} />
      {loading ? (
        <Loading />
      ) : (
        <InfiniteScroll
